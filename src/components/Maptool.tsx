...
const MapTool = () => {
  ...
  useEffect(() => {
    ...
    map.current.on(L.Draw.Event.CREATED, (event: L.LeafletEvent) => {
      const drawEvent = event as L.LeafletEvent & { layer: L.Layer; layerType: string };
    ...
    touristLocations.forEach(location => {
      const riskColor = location.riskPercentage > 30 ? '#ef4444' : 
                       location.riskPercentage > 15 ? '#f59e0b' : '#10b981';
      ...
      L.marker(location.coordinates, { icon: customIcon })
        .bindPopup(`
          <div style="text-align: center; padding: 8px; min-width: 200px;">
            <h3 style="color: #2A777C; margin: 0 0 8px 0; font-size: 14px;">${location.name}</h3>
          ...
          <div className="col-span-3 space-y-4">
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="risk-percentage">Risk Percentage (0-100)</Label>
                  <Input
                    id="risk-percentage"
                    type="number"
                    min="0"
                ...
                {touristLocations
                  ...
                  <div key={location.id} className="p-3 border rounded-lg space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge 
                        variant={location.riskPercentage > 30 ? "destructive" : 
                                location.riskPercentage > 15 ? "default" : "secondary"}
                        className="text-xs"
                      >
                        {location.riskPercentage}% Risk
                      </Badge>
                    ...
                    <div className="text-xs text-muted-foreground">
                      By: {location.addedBy} • {location.timestamp.toLocaleDateString()}
                    </div>
                  </div>
                ))}
              </CardContent>
          ...
          <div className="col-span-9">
            <Card className="h-full">
              ...
              <CardContent className="p-4 h-[calc(100%-80px)]">
                <div ref={mapContainer} className="h-full rounded-lg border border-border" />
              </CardContent>
            </Card>
          </div>
        </div>
